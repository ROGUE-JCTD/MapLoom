describe('StoryLegendDirective', function() {
  ...
  describe('layer-added', function() {
    ...
    it('gets all layers', inject(function() {
      expect(scope.layerAliases.length).toBe(2);
    }));
    it('the layers have an id', inject(function() {
      expect(scope.layerAliases[0]).toEqual(jasmine.objectContaining({
        id: 1
      }));
    }));
    ...
    it('the layers can have a titleAlias', inject(function() {
      expect(scope.layerAliases[1]).toEqual(jasmine.objectContaining({
        title: 'O.B.'
      }));
    }));
  });
  describe('#saveMasking', function() {
    ...
    it('saves the masking to the configration', function() {
      scope.layerAliases[0].title = 'Test';
      scope.saveMasking();
      expect(scope.layerAliases[0].layer.get('metadata').config.titleAlias).toBe('Test');
    });
  });
});
